...
def compress_image(img, w=128, h=128):
    ...
    print('成功保存为png格式, 压缩为128*128格式图片')
...
def logo_watermark(baseImage, logo_path):
    '''
    添加一个图片水印,原理就是合并图层，用png比较好
    '''
    logoim = Image.open(logo_path)
    bw, bh = baseImage.size
    lw, lh = logoim.size
    baseImage.paste(logoim, (bw - lw, bh - lh))
    baseImage.save('test3.jpg', 'JPEG')
    print('logo水印组合成功')
