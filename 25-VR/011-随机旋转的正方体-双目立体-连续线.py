...
def draw_cube(cube):
    ...
    right_img = np.zeros((sz, sz, 3), dtype=np.uint8)
    r = sz / 2 / np.sqrt(3) - 3  # 正方体的边长
    aa = np.round((cube + np.array([-5, 0, 0])) * r).astype(np.int32)  # 新的正方体的每个顶点的位置，把正方体往远处推
    eye_dis = 0.6
    left_eye, right_eye = (0, -eye_dis / 2, 0), (0, eye_dis / 2, 0)
    for f, t in edges:
        fp = ball2rec(*aa[f], sz, sz)
        tp = ball2rec(*aa[t], sz, sz)
        point_count = np.linalg.norm(np.array(fp) - np.array(tp))
        for v in np.linspace(aa[f], aa[t], int(point_count)):
            row, col = ball2rec(*v - left_eye, sz, sz)
            left_img[row][col] = 255
            row, col = ball2rec(*v - right_eye, sz, sz)
            right_img[row][col] = 255
